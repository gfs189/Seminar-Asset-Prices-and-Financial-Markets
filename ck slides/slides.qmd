...
**Step 1 — Estimate the monthly cost per stock**

From CRSP daily data, compute each stock's daily proportional **bid-ask half-spread**:

$$c_{i,d} = \frac{\text{ask}_{i,d} - \text{bid}_{i,d}}{2 \cdot \text{mid}_{i,d}}$$

Aggregate to a **monthly cost** by taking the **median** of daily half-spreads within each month — robust to stale or missing quotes.

:::
::: {}

**Step 2 — Apply costs to each portfolio leg**

Assume **full monthly rebalancing** (upper bound on turnover). Compute the rank-weighted average monthly half-spread for each leg:

$$\bar{c}^L = \sum_i w_i^L\, c_i, \qquad \bar{c}^H = \sum_i w_i^H\, c_i$$

Apply the same beta-inverse scaling as the BAB return — the round-trip cost (×2) per unit of BAB capital is:

$$TC^L = \frac{2\,\bar{c}^L}{\beta^L}, \qquad TC^H = \frac{2\,\bar{c}^H}{\beta^H}$$

::: {.highlight-box}
**Net BAB return** $= r^{BAB} - TC^L - TC^H$
:::
